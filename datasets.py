...
if config.DATASET == "NUSWIDE":
    ...
    for label in range(label_set.shape[1]):
        ...
        # if first:
        #     test_index = index[:50]
        #     train_index = index[50:200]
        #     first = False
        # else:
        #     ind = np.array([i for i in list(index) if i not in (list(train_index)+list(test_index))])
        #     test_index = np.concatenate((test_index, ind[:50]))
        #     train_index = np.concatenate((train_index, ind[50:200]))
...
if config.DATASET == "MSCOCO":
    ...
    for label in range(label_set.shape[1]):
        ...
        if first:
            test_index = index[0:30]
            train_index = index[30:130]
            first = False
        else:
            ind = np.array([i for i in list(index) if i not in (list(train_index) + list(test_index))])
            test_index = np.concatenate((test_index, ind[:30]))
            train_index = np.concatenate((train_index, ind[30:130]))
        # if first:
        #     test_index = index[:50]
        #     train_index = index[50:200]
        #     first = False
